...
func think(brain Brain,sensor_data [NUM_NEURONS]byte) int {
	//fmt.Println("IN THINK sensor_data: ",sensor_data)
	...
	//new_angle_index = 0
        ...
        knt := 0
	for ik:=0;ik<NUM_NEURONS;ik++ {
            knt = knt + int(sensor_data[ik])
	}

        if knt <= 0 {
            sensor_data[getRandomInt(0,NUM_NEURONS)] = 1
    	}
